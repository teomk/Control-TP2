...
Para caracterizar el desempeño se utilizan las métricas IAE (\textit{integral absolute error}), ISE (\textit{integral squared error}) y RMS (\textit{root mean square}) de la señal de control. En este caso se usa RMS porque se mide la magnitud media de $\tau_r(t)$; si se aplicara esta misma idea al error de seguimiento, correspondería hablar de RMSE.

\input{tables/parte_a_metricas}

\subsection{Discusiones}

La embarcación logra seguir la referencia de rumbo por tramos. En cada cambio de consigna aparece un transitorio, pero la salida eventualmente converge al nuevo valor de referencia con error estacionario nulo gracias al integrador.
...
Se puede observar en la figura~\ref{fig:parte-a-seguimiento-comparacion} que una selección de polos más a la izquierda genera una respuesta más rápida, pero con picos de control $\tau_r$ más altos que podrían superar a las capacidades del actuador. Por el contrario, polos más cercanos al origen generan una respuesta más lenta y suave, con menor exigencia sobre el actuador.

En la práctica, conviene elegir polos que resulten en una respuesta lo suficientemente veloz y suave, sin exigir al actuador por sobre su límite. Para este caso no se conocen esos límites, pero se considera que el ejemplo graficado en naranja es un buen punto medio.

Para definir la velocidad ideal, en el caso del barco, también sería necesario considerar que movimentos demasiado bruscos podrían desestabilizar la embarcación.
\FloatBarrier
